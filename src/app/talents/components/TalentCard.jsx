  ...
  return (
     <Card className="shadow-none border relative border-gray-200 hover:shadow-lg ">
        ...
        <CardContent className="flex flex-col justify-center items-center  p-6"> 
            <Link href={`/talents/${talent?.talent?._id}`} className='relative flex justify-center items-center bg-orange-400 rounded-full w-40 h-40 mr-4'>  
            ...
            <Link href={`/talents/${talent?.talent?._id}`} className='flex flex-col justify-center items-center'>
                <h3 className="text-lg font-semibold text-center">{talent?.talent?.fullName}</h3>
                <p className='text-sm text-gray-500 '>{ talent?.talent?.headline }</p>
